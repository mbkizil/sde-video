...
def get(dataset):
    if dataset == 'mmnist':
        data_train = MovingMNIST(train=True, data_root='/kuacc/users/mkizil19/hpc_run/sdevideo/sdevideo/VideoAsAStochasticProcess-main/sde/data', seq_len=20, num_digits=2, deterministic=False)
        data_val = MovingMNIST(train=False, data_root='/kuacc/users/mkizil19/hpc_run/sdevideo/sdevideo/VideoAsAStochasticProcess-main/sde/data', seq_len=20, num_digits=2, deterministic=False)
        image_size = 64
        num_channels = 1
...
def fast_collate_for_prediction(batch):
    ...
    if isinstance(batch[0][0], tuple):
        ...
        targets = torch.zeros(flattened_batch_size, dtype=np.float32)
        tensor = torch.zeros((flattened_batch_size, *batch[0][0][0].shape), dtype=np.float32)
        ...
        targets = torch.tensor([b[1] for b in batch], dtype=np.float32)
        assert len(targets) == batch_size
        tensor = torch.zeros((batch_size, *batch[0][0].shape), dtype=np.float32)
        for i in range(batch_size):
            tensor[i] += torch.from_numpy(batch[i][0])
        return tensor, targets
    elif isinstance(batch[0][0], torch.Tensor):
        targets = torch.zeros((batch_size, *batch[1][0].shape), dtype=np.float32)
        assert len(targets) == batch_size
        tensor = torch.zeros((batch_size, *batch[0][0].shape), dtype=np.float32)
        for i in range(batch_size):
...
def get(batch_size, val_batch_size=32, data_root='/kuacc/users/mkizil19/hpc_run/sdevideo/sdevideo/original/sde/data', num_workers=2, data_name='mnist',
    ...
    #test_set = torch.Tensor.numpy(test_set)
    ...
    return dataloader_train, dataloader_vali, dataset_kwargs
